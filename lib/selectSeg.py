...
class MakeFolder(tk.Toplevel):
    def __init__(self, master, title=None):
        ...
        self.label = tk.Label(self, text="请输入工作空间名称，将在桌面上创建此目录 ", font=self.font).pack(side=tk.TOP)
        self.entry = tk.Entry(self, textvariable=self.dir_name,
                              font=self.font).pack(fill=tk.BOTH, expand=1)
    ...
    def centerPos(self):
        ...
        x = (ws / 2) - (400 / 2)
        y = (hs / 2) - (76 / 2)
        self.geometry('%dx%d+%d+%d' % (400, 76, x, y))
        self.wm_attributes('-topmost', 1)

    def makeButtons(self):
        tk.Button(self, text="创建/打开", font=self.font, command=lambda: self.openWs(
    ...
    def openWs(self, dir_name):
        ...
        if not dir_name:
            ...
            if language == 'en_US':
                ...
                print("system language is neither Chinese nor English, set desktop = \'Desktop\'")
                desktop = 'Desktop'
            ...
            try:
                ...
                if not os.path.isdir(ws_dir):
                    ...
                    os.mkdir(ws_dir)
                    text = '工作空间<%s>已准备就绪，请打开kml文件' % dir_path
                    tm.showinfo('提示', text)

                else:
                    print('Dir <%s> exist.' % ws_dir)
                    text = '工作空间目录<%s>已存在，如果目录存在kml文件，将不能打开其他kml文件' % ws_dir
                    tm.showinfo('提示', text)
...
class App(tk.Frame):
    ...
    def __init__(self, root):
        ...
        self.filemenu.add_command(label="创建/打开工作空间", font=self.font, command=self.openWorkspace)
        self.filemenu.add_command(label="打开kml文件", font=self.font, command=self.openFile)
        self.filemenu.add_command(label="保存暂存路段", font=self.font, command=self.saveAllSeg)
        self.menubar.add_cascade(label="文件", font=self.font, menu=self.filemenu)

        self.editmenu = tk.Menu(self.menubar, tearoff=False)
        self.editmenu.add_command(label="删除路段", font=self.font, command=self.deleteSeg)
        self.editmenu.add_command(label="生成路网", font=self.font, command=self.genTopologyRoad)
        self.menubar.add_cascade(label="编辑", font=self.font, menu=self.editmenu)
        ...
        self.helpmenu.add_command(label="操作提示", font=self.font, command=self.helpMSG)
        self.helpmenu.add_command(label="关于", font=self.font, command=self.aboutMSG)
        self.menubar.add_cascade(label="帮助", font=self.font, menu=self.helpmenu)
    ...
    def getPoints(self):
        ...
        try:
            ...
            fileName = td.askopenfilename(initialdir=desktop_path,
