...
class Config:
    ...
    def __init__(self, ws_dirs):
        ...
        self.dis_delta = 8.5  # self.radius = 8m


def getDocPaths(dirs_name):
    # txt file

    filepathList = []

    for each in dirs_name:

        for maindir, subdir, fileListStr in os.walk(each):
            fileList = []
...
def genRoad(ws_dirs):
    ...
    filepath_in = getDocPaths(config.dirs_in)
...
if __name__ == '__main__':
    ...
    desktop_ws = os.path.join(home, 'Desktop', 'seg_merge')
    input_dirs_temp_seg = ['/home/mengze/Desktop/temp_seg1', '/home/mengze/Desktop/temp_seg2']
    output_seg = os.path.join(desktop_ws, 'output')

    if not os.path.isdir(output_seg):
        print('Make dir <%s>.' % output_seg)
        os.makedirs(output_seg)

    else:
        print('Delete and make new dir <%s>.' % output_seg)
        shutil.rmtree(output_seg)
        os.makedirs(output_seg)

    ws_dirs = [desktop_ws, input_dirs_temp_seg, output_seg]
    genRoad(ws_dirs)
